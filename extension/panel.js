...
const nsInput = $('ns-filter');
const searchInput = $('search');
const pauseBtn = $('pause');
const clearBtn = $('clear');
const echoSelect = $('console-echo');
...
function passFilter(e) {
    ...
    if (minIdx >= 0 && LEVEL_ORDER.indexOf(e.level) < minIdx)
        return false;
    const ns = nsInput.value.trim();
    if (ns && !e.ns.toLowerCase().includes(ns.toLowerCase()))
        return false;
    const q = searchInput.value.trim().toLowerCase();
    if (q && !e.msg.toLowerCase().includes(q) && !e.ns.toLowerCase().includes(q))
        return false;
    return true;
}
...
function appendRow(e) {
    ...
    while (rowsEl.children.length > MAX_DOM_ROWS)
...
async function poll() {
    const { result, isException } = await evalPage(`(typeof __ELITJS_LOG__ === 'undefined') ? null : __ELITJS_LOG__.dump(${lastSeq})`);
    ...
    if (dump.entries.length > 0) {
        ...
        if (all.length > MAX_MEMORY)
            all.splice(0, all.length - MAX_MEMORY);
